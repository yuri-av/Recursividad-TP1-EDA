...
// ejercicio 7: ACTUALIZADO AL .H (retorna char* para que el puente no se queje)
char *ondaDigital(char seniales[])
{
    if (*seniales == '\0')
    {
        return ""; // Para cumplir con el char* del prototipo sin alterar la logica
    }
    ...
    {
        printf(" ");
        if (*seniales != *(seniales + 1))
        {
            printf("|");
        }
    }
    ondaDigital(seniales + 1);

    return ""; // Para cumplir con el char*
}
...
void subconjuntosQueSumanN(int conjunto[], int tamano, int n, char **output)
{
    ...
    *output = malloc(1024 * sizeof(char)); // Reservamos la memoria
    ...
    buscarSubconjuntos(conjunto, tamano, n, 0, subconjuntoActual, 0, output);
}

// Ejercicio 9: ACTUALIZADO AL .H
